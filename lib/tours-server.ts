...
function ensureDataFile() {
  ...
  if (!fs.existsSync(DATA_FILE)) {
    fs.writeFileSync(DATA_FILE, JSON.stringify(SEED_TOURS, null, 2))
  }
}
...
export function getTourById(id: number): Tour | undefined {
  return getTours().find(tour => tour.id === id)
}
...
export function getToursByContinent(continent: string): Tour[] {
  return getTours().filter(tour => tour.continent.toLowerCase() === continent.toLowerCase())
}

export function getToursByType(type: string): Tour[] {
  return getTours().filter(tour => tour.type.toLowerCase() === type.toLowerCase())
}

export function searchTours(query: string): Tour[] {
  const lowercaseQuery = query.toLowerCase()
  return getTours().filter(tour =>
...
export function createTour(data: Omit<Tour, "id" | "created_at" | "updated_at">): Tour {
  const tours = getTours()
  const nextId = tours.length > 0 ? Math.max(...tours.map(t => t.id)) + 1 : 1
  const tour: Tour = {
    ...data,
    id: nextId,
    created_at: new Date().toISOString(),
...
export function updateTour(id: number, data: Partial<Tour>): Tour | null {
  const tours = getTours()
